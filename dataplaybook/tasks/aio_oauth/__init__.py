...
def task_oauth_authenticate(_, client_id, client_secret, scopes=None):
...
@cv.task_schema({vol.Required("url"): str}, kwargs=True, target=True)
...
@cv.task_schema({vol.Required("filename"): str}, kwargs=True, target=True)
